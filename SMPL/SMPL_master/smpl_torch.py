...
class SMPLModel(Module):
    def __init__(
        self, device=None, model_path="/home/dcor/giladd/arm_exps/SMPL/model.pkl"
    ):
    ...
    def rodrigues(r):
        ...
        i_cube = (
            torch.eye(3, dtype=torch.float64).unsqueeze(dim=0)
    ...
    def forward(self, betas, pose, trans, simplify=False):
        ...
        v_shaped = (
            torch.tensordot(self.shapedirs, betas, dims=([2], [0])) + self.v_template
        )
        ...
        if simplify:
            ...
            v_posed = v_shaped + torch.tensordot(
                self.posedirs, lrotmin, dims=([2], [0])
            )
        ...
        results = stacked - self.pack(
            torch.matmul(
                ...
                torch.reshape(
                    torch.cat(
                        (J, torch.zeros((24, 1), dtype=torch.float64).to(self.device)),
                        dim=1,
                    ),
                    (24, 4, 1),
                ),
        ...
        rest_shape_h = torch.cat(
            (
                v_posed,
                torch.ones((v_posed.shape[0], 1), dtype=torch.float64).to(self.device),
            ),
            dim=1,
        )
...
def test_gpu(gpu_id=[0]):
    ...
    result = model.forward(betas, pose, trans)
